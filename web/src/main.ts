...
class GeoV1Application {
  ...
  /**
   * Start camera synchronization between Cesium and deck.gl
   * This runs at 60 FPS to keep the deck.gl GlobeView in sync with Cesium camera
   */
  private startCameraSync() {
    if (!this.viewer || !this.renderWorker) {
      console.warn('Cannot start camera sync: viewer or renderWorker not initialized');
      return;
    }
    ...
    const syncCamera = () => {
      ...
      // Convert Cesium position to deck.gl GlobeView coordinates
      const longitude = cartographic.longitude * (180 / Math.PI);
      const latitude = cartographic.latitude * (180 / Math.PI);
      const cesiumAltitude = cartographic.height; // meters above ellipsoid

      // Convert Cesium altitude to deck.gl GlobeView altitude (relative units)
      const altitude = this.cesiumAltitudeToGlobeViewAltitude(cesiumAltitude);

      // Send camera update to render worker
      this.renderWorker.postMessage({
        type: 'camera-sync',
        payload: {
          longitude,
      ...
      // Continue sync loop at 60 FPS
      requestAnimationFrame(syncCamera);
    };

    // Start the sync loop
    requestAnimationFrame(syncCamera);
    console.log('✓ Camera sync started');
  }

  /**
   * Convert Cesium altitude (meters) to deck.gl GlobeView altitude (relative units)
   * GlobeView altitude is relative to the viewport size (1 unit ≈ viewport height)
   *
   * @param cesiumAltitude - Height in meters above the ellipsoid
   * @returns Relative altitude for deck.gl GlobeView
   */
  private cesiumAltitudeToGlobeViewAltitude(cesiumAltitude: number): number {
    ...
    // Normalize altitude relative to Earth's radius
    // This gives us a 0-1 range where 1 = Earth radius
    const normalizedAltitude = cesiumAltitude / EARTH_RADIUS;

    // Scale factor found empirically to match visual expectations
    // Higher value = closer view (smaller altitude value in deck.gl means closer)
