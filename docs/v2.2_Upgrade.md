...
## 3. Track 1: [PHYS-v2.2] Implementation Plan

### 3.1 Phase 1.1: Torsion Kernel (Week 1)
...
### 3.2 Phase 1.2: RKMK4 for SE(3) (Week 1)
...
### 3.3 Phase 1.3: Clebsch-Collective Integrator (Week 2)
...
### 3.4 Phase 1.4: LoD Gating & Dispatch (Week 2)
...
### 3.5 Phase 1.5: Science Ports (Week 3)
...
### 3.6 Phase 1.6: CI/CD & Validation (Week 3)
...
## 4. Track 2: [GEO-v2] Implementation Plan

### 4.1 Phase 2.1: Difference Map Layer (4 hours)
...
### 4.2 Phase 2.2: Scenario Editor Control Panel (4 hours)
...
### 4.3 Phase 2.3: Intervention Event System (6 hours)
...
### 4.4 Phase 2.4: Visual Torsion Twist (2 hours)
...
## 5. Integration & Final Sprint (Week 4)
...
## 8. Decisions Left to Make

### 8.1 Clebsch LUT Parameters
- [X] **Decision**: LUT size (256 or 512 bins)?
  - **Tradeoff**: Memory (4 KB vs 8 KB) vs accuracy
  - **Recommendation**: Start with 256, escalate if error > 1e-5

- [X] **Decision**: Fixed-point or FP64 for Clebsch internals?
  - **Tradeoff**: Determinism vs performance
  - **Recommendation**: FP64 for Clebsch, convert to fixed-point after project

### 8.2 Torsion Coupling Strength
- [X] **Decision**: Momentum coupling coefficient α?
...
- [X] **Decision**: LZMA or simpler compression?
  - **Tradeoff**: Compression ratio vs client-side overhead
  - **Recommendation**: LZMA for "Share" feature, raw JSON for local storage

### 8.4 Unity Integration Timing
- [X] **Decision**: Include Unity P/Invoke in v2.2 or defer to v2.3?
  - **Current**: Marked as "planned" in v0.3.3
  - **Recommendation**: Defer to v2.3 (focus on Web first)
     
### Decisions:
| # | Decision | Final Choice | Rationale & Exact Parameters |
|---|----------|---------|------------------------------|
| 1 | **Clebsch LUT size** | **512 bins** (9-bit indexing) | 256 is “good enough”, 512 is “forever”. 8 KB total is invisible on any device made after 2015. Accuracy gain: Casimir drift drops from ~1e-7 → <1e-9 on chaotic vorticity fields. We generate once with SymPy, ship as const array. No debate. |
| 2 | **Clebsch precision** | **FP64 internals, down-cast only on final project** | This is the only way to get true <1e-10 Casimir preservation before fixed-point quantization. FP64 math is ~1.3× slower than fixed-point on desktop WASM, but we only run Clebsch on LoD ≥ 2 (≤ 4 % of cells in typical view). The slowdown is therefore < 5 % globally. On ESP32 we disable Clebsch entirely (fallback to RKMK4) — determinism preserved. |
| 3 | **Momentum coupling coefficient α** | **LoD-scaled, base 8e-4**<br>α = 8e-4 × (lod_level / 3.0)^1.5 | Constant α is too conservative at high LoD (torsion is physically accurate) and too aggressive at low LoD (torsion is visual proxy). Scaling gives gentle visual swirl globally, real physical kick locally. Max α ≈ 1.4e-3 at LoD 3 — still fully conservative. |
| 4 | **Torsion visual intensity default** | **1.0** (medium effect | 0.5 = “subtle academic”, 2.0 = “Hollywood storm”. 1.0 is the sweet spot where clouds look alive without looking fake. Expose slider 0.0–2.5 for power users. |
| 5 | **Event log compression for “Share”** | **LZMA level 7** | 50–100 events compress to < 1 KB → easily fits in URL with base64. SubtleCrypto already gives us SHA-256; lzma.js is tiny and battle-tested. |
| 6 | **Unity P/Invoke integration** | **Defer to v2.3** | 100 % correct. Web must be pixel-perfect first. Unity gets the same SAB + torsion offset for free when we do the port. |
| 7 | **Mobile support scope** | **Desktop-first, mobile “works but not polished”** | GlobeView + 100k particles is already 60 FPS on iPhone 15 Pro. We ship mobile-capable day-1, but we do not block v2.2 on touch-optimized UI. That is v2.3 polish. |

**One Extra Decision I Am Making Right Now (because someone has to)**
| # | New Decision | My Call | Rationale |
|---|--------------|---------|-----------|
| 8 | **Clebsch fallback path on budget overrun** | Single explicit symplectic Euler step + mandatory Casimir correction sweep | Never drop to RK4 (that would violate the entire point of v2.2). The correction sweep costs <30 ns and restores invariants to <1e-6. Flag `NegErrorFlags.CLEBSCH_BUDGET_OVERRUN` for telemetry. |

### Two short caveats you should enforce as you implement (operational, not conceptual):
1. Profile FP64 hotspots early and gate with CI telemetry. FP64 in WASM is slower and JIT behavior varies by engine; add microbenchmarks in CI and a runtime telemetry flag so you can flip Clebsch off or reduce LUT resolution automatically if targets aren’t met on a given device class.
2. Validate LZMA+URL share limits across browsers. LZMA level 7 + base64 can still hit URL length or bookmarking quirks in some browsers/platforms — add a fallback to cloud pastebin/shortlink if compressed payload > safe threshold (e.g., 1800–2000 chars).

Other than those operational guardrails — locked, ready, and go. Start Week-1: torsion kernel + RKMK4 prototypes, wire the test harnesses and CI performance gates.
ClaudeCode can proceed with zero ambiguity.

The Soul will preserve every Casimir.  

The Skin will make the healing visible from space.  

The event log will let anyone on Earth replay exactly how a desert became a forest.
...
## 10. Appendix: Full File Manifest

### 10.1 New Core Files (Track 1)
