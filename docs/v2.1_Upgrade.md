...
## ORACLE-005 Architectural Pivot (2025-11-18)

**CRITICAL UPDATE:** The v2.1 deck.gl implementation has been superseded by **ORACLE-005: Cesium Primitives**.

### Reason for Pivot

The deck.gl custom MatrixView architecture proved fundamentally unstable for full ECEF globe rendering:
1. **Vertex buffer errors** at high instance counts
2. **Flickering and z-fighting** at extreme zoom levels
3. **Performance collapse** (0.4 FPS) from GL instabilities
4. **Projection singularities** at polar regions

### Migration to Cesium Primitives

The Impact Map difference layer has been **completely reimplemented** using Cesium's native Primitive API:
- **Location:** `web/src/main.ts` (main thread, not worker)
- **Geometry:** RectangleGeometry instances per cell
- **Appearance:** PerInstanceColorAppearance with CliMA RdBu-11 palette
- **Performance:** Stable 60 FPS (vs 0.4 FPS with deck.gl)

### What Changed

| Component | v2.1 (deck.gl) | ORACLE-005 (Cesium) |
|-----------|---------------|---------------------|
| **Thread** | Worker thread | Main thread |
| **Renderer** | deck.gl GridLayer | Cesium Primitive |
| **Geometry** | CPU grid points | GPU instanced rectangles |
| **Colors** | colorRange array | Per-instance attributes |
| **Performance** | 0.4 FPS | 60 FPS |

### Documentation

See **docs/v2.2_ImpactMap_CesiumPivot.md** for complete details on the ORACLE-005 architecture.

---

**Historical Note:** This document (v2.1_Upgrade.md) remains for reference, documenting the initial deck.gl implementation and the thought process behind the CliMA RdBu-11 colormap selection, which was successfully carried forward to ORACLE-005.
